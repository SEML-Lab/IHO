...
class LLaDAWrapper:
    ...
    def __init__(
        ...
        self.padding_token_id = int(self.tokenizer.pad_token_id)
    ...
    def mask_tokens(
        ...
        if masking_mode == "all":
            ...
            eligible_mask[:, :attack_size] = True
        elif masking_mode == "attack":
            mask_block = token_ids.new_full((batch_size, attack_size), self.mask_token_id)
    ...
    def _get_num_transfer_tokens(self, mask_index: torch.Tensor, attack_steps: int) -> torch.Tensor:
        mask_num = mask_index.sum(dim=1, keepdim=True)
        base = mask_num // attack_steps
        remainder = mask_num % attack_steps
    ...
    def predict_masked(
        ...
        num_transfer_tokens = self._get_num_transfer_tokens(mask_index_initial, attack_steps)
        ...
        for step in range(attack_steps):
            ...
            for b in range(batch_size):
                ...
                if (
                    k == 0
                    and step < attack_steps - 1 - number_global_remask
                    ...
                    if len(unknown_indices) >= number_global_remask:
                        if global_remasking == "random":
                            rnd = torch.randperm(len(unknown_indices), device=self.device)[
                                :number_global_remask
                            ]
                            ...
                            raise ValueError(
                                f"Unknown global remasking strategy: {global_remasking}"
                            )
